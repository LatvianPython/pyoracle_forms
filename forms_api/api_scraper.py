...
def parse_headers(api_version):
    ...
    abbreviations = {
        ...
        for abbreviation, object_type
        in abbreviations.items()
        if object_type in objects.keys()
    }

    macro_regex = re.compile(r'#define d2f\w+?[gs]_(\w+?)\(.+(?:(?:Get)|(?:Set))(.+?)Prop.+,(\w+),val\)')
    ...
    for abbreviation, object_type in abbreviations.items():
        ...
        for macro in macros:
            ...
            if match:
                ...
                object_properties[object_type].add((prop_name, prop_type.lower(), prop, properties[prop]))
    ...
    parsed = {
        obj_name: {
            ...
            'properties': [
                ...
                for (short_name, data_type, macro_name, property_name)
                in properties
            ]}
        for (obj_name, obj_type), properties
